...
//========================================================================================
//! \file ti.cpp
//! \brief Problem of thermal instability. Basic setup from `linear_wave.cpp`
//!
//========================================================================================
...
namespace {
  ...
  Real tau, tau_heat, alpha, temp_floor;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  tau_heat = pin->GetOrAddReal("problem", "tau_heat", 0.0);
  alpha = pin->GetOrAddReal("problem", "alpha", 0);
  temp_floor = pin->GetOrAddReal("problem", "temp_floor",1e-5);
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k) {
    for (int j = pmb->js; j <= pmb->je; ++j) {
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        if (temp > temp_floor) {
          cons(IEN,k,j,i) -= dt / tau * prim(IDN,k,j,i) * pow(temp / temp_floor,alpha) / (g - 1.0);
        }
        const(IEN,k,j,i) += dt/tau_heat / (g - 1.0);
      }
    }
  }
  return;
